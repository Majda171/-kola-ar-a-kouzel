-- BRADAVICE v41 — KOMPLETNÍ DATABÁZOVÁ MIGRACE
-- Spusť v Supabase SQL Editoru po SUPABASE-V29-DENNI-VYZVA.sql a SUPABASE-V35-ADMIN-PROFILY-AVATARY.sql.
-- Tento jediný soubor obsahuje i opravenou v37 (chyba 42P13), Hagridův úkol a všechny databázové novinky v40 a v41.
...
-- Ředitelský účet Mafaldy. Heslo zůstává výhradně v Supabase Auth a v kódu se neukládá.
update public.profiles p
   set is_admin=true,
       first_name=case when coalesce(trim(p.first_name),'')='' then 'Mafalda' else p.first_name end,
       last_name=case when coalesce(trim(p.last_name),'')='' then 'Robertsová' else p.last_name end,
       updated_at=now()
  from auth.users u
 where p.id=u.id and lower(coalesce(u.email,''))='bradavice_info@seznam.cz';
...
begin
  ...
  case p_activity_key
    ...
    when 'herbarium-prytova' then pts:=25; label:='Profesorka Prýtová: Herbář Bradavic';
    when 'lupin-boggart' then pts:=20; label:='Praktická zkouška s bubákem';
...
-- D) NOVINKY v41
-- ============================================================
-- Snape při zkaženém lektvaru náhodně odečte 5 až 50 bodů.
-- Menší tresty jsou záměrně pravděpodobnější než nejvyšší.
create or replace function public.v41_snape_penalty()
returns integer
language plpgsql
security definer
set search_path=public
as $$
declare uid uuid:=auth.uid(); r double precision:=random(); pts integer;
begin
  if uid is null then raise exception 'Student není přihlášen.'; end if;
  if exists(select 1 from public.profiles where id=uid and banned_at is not null) then raise exception 'Účet je vyloučen ze školy.'; end if;
  pts:=case
  ...
  perform public.add_points_internal(uid,-pts,'Profesor lektvarů – zkažený lektvar','v41','snape-penalty');
  return -pts;
end;
$$;
