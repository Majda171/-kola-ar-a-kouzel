(()=>{
  ...
  const HERB_PREFIX='bradavice_herbology_v4366';
  const LEGACY_PREFIXES=['bradavice_herbology_v4362','bradavice_herbology_v4354'];
  const plantDefs={
  ...
  const questKey=()=>window.BradaviceAchievements?.scopedKey?.(QUEST_KEY)||`${QUEST_KEY}_${scopeId}`;
  const readQuest=()=>readJson(questKey(),{});
  ...
  const cleanId=v=>String(v||'').trim().replace(/[^a-zA-Z0-9@._-]/g,'_');
  let scopeId='guest',scopeAliases=['guest'];
  function refreshIdentity(user=null){
    const st=readJson(STUDENT_KEY,null);
    const raw=[user?.id,st?.supabaseUserId,st?.email].map(cleanId).filter(Boolean);
    scopeAliases=[...new Set(raw.length?raw:['guest'])];
    scopeId=scopeAliases[0]||'guest';
    return scopeId;
  }
  refreshIdentity();
  const herbKey=()=>`${HERB_PREFIX}_${scopeId}`;
  function mergeHerbStates(){
    const merged=defaultState(),keys=[];
    for(const id of scopeAliases){
      keys.push(`${HERB_PREFIX}_${id}`);
      for(const pre of LEGACY_PREFIXES)keys.push(`${pre}_${id}`);
    }
    if(scopeId==='guest')keys.push(...LEGACY_PREFIXES,HERB_PREFIX);
    for(const key of [...new Set(keys)]){
      const x=readJson(key,null);if(!x||typeof x!=='object')continue;
      merged.accepted=merged.accepted||Boolean(x.accepted);
      merged.done=merged.done||Boolean(x.done);
      merged.mandAccepted=merged.mandAccepted||Boolean(x.mandAccepted);
      merged.mandDone=merged.mandDone||Boolean(x.mandDone);
      for(const id of ids)if(x.found?.[id])merged.found[id]=true;
      if(x.updatedAt&&(!merged.updatedAt||x.updatedAt>merged.updatedAt))merged.updatedAt=x.updatedAt;
    }
    return merged;
  }
  function readHerb(){const s=mergeHerbStates();s.found||={};return s}
  function saveHerb(s){
    s={...defaultState(),...s,found:{...(s?.found||{})},updatedAt:new Date().toISOString()};
    writeJson(herbKey(),s);
    for(const id of scopeAliases.slice(1))writeJson(`${HERB_PREFIX}_${id}`,s);
  }
  function mirrorQuest(id){const q=readQuest();q[id]={done:true,at:new Date().toISOString()};writeJson(questKey(),q)}
  // Supabase je zdroj pro DOPLNĚNÍ uloženého postupu, ale nesmí lokální
  // stav konkrétního studenta vracet zpět. To je důležité hlavně těsně po
  // přijetí úkolu: zápis na server může dorazit o chvíli později než další
  // načtení stránky. Studentův accepted/found stav proto pouze povyšujeme na
  // true; nikdy jej při synchronizaci nemažeme.
  function reconcileFromQuestRows(){
    const q=readQuest(),s=readHerb();let changed=false;
    const rowFound=Object.fromEntries(ids.map(id=>[id,done(q,plantQ(id))]));
    const rowAccepted=done(q,qid.accept)||done(q,qid.done)||ids.some(id=>rowFound[id]);
    const rowDone=done(q,qid.done);
    const rowMandDone=done(q,qid.mandDone);
    const rowMandAccepted=done(q,qid.mandAccept)||rowMandDone;
    const promote=(key,val)=>{if(val&&!s[key]){s[key]=true;changed=true}};
    promote('accepted',rowAccepted);promote('done',rowDone);promote('mandAccepted',rowMandAccepted);promote('mandDone',rowMandDone);
    for(const id of ids){if(rowFound[id]&&!s.found[id]){s.found[id]=true;changed=true}}
  ...
  const herbAccepted=()=>Boolean(reconcileFromQuestRows().accepted);
  const herbDone=()=>Boolean(reconcileFromQuestRows().done);
  const mandAccepted=()=>Boolean(reconcileFromQuestRows().mandAccepted);
  const mandDone=()=>Boolean(reconcileFromQuestRows().mandDone);
  const found=id=>Boolean(reconcileFromQuestRows().found[id]);
  ...
  async function completeRemote(id){
    ...
    finally{reconcileFromQuestRows()}
  }
  async function syncBeforeRender(){
    try{
      if(DB?.client&&DB?.hydrateStudent)await DB.hydrateStudent();
      let user=null;try{user=await DB?.getUser?.()}catch{}
      refreshIdentity(user);
      const before=readHerb();saveHerb(before);
      if(DB?.client&&DB?.hydrateProgress)await DB.hydrateProgress();
    }catch(e){console.warn('Prýtová – načtení postupu selhalo:',e);refreshIdentity()}
    reconcileFromQuestRows();
  }
  ...
  function initGreenhouse(){
    ...
    function render(){
      reconcileFromQuestRows();renderProgress();const n=foundCount();
      if(herbDone()){A?.award?.('herbarnik',{silent:true});herbDialog.innerHTML='<strong>Herbář je odevzdaný.</strong> Profesorka Prýtová má všech pět vzorků bezpečně zapsaných.';herbAction.hidden=true}
      ...
      if(mandDone()){A?.award?.('mandragorovy-pestitel',{silent:true});mandDialog.innerHTML='<strong>Mandragorový úkol je splněný.</strong> Výsledek je uložený.';mandAccept.hidden=true;trans.hidden=true}
      else if(!mandAccepted()){mandDialog.textContent='Přijmi výzvu a potom máš 60 sekund na co nejvíc správně přesazených mandragor.';mandAccept.hidden=false;mandAccept.disabled=false;trans.hidden=true}
    ...
    herbAction?.addEventListener('click',()=>{
      ...
      if(foundCount()===5&&!herbDone()){setFlag('done',qid.done);A?.award?.('herbarnik');toast('Herbář odevzdán.');render();completeRemote(qid.done);return}
      render();
    });
  ...
  (async()=>{await syncBeforeRender();mountPlants();initGreenhouse();window.addEventListener('bradavice:progress-synced',()=>{refreshIdentity();reconcileFromQuestRows();refreshMountedPlants();if(page==='skleniky.html')document.getElementById('herbQuestModal')?._herbRender?.()})})();
})();
