(()=>{
  ...
  const HERB_PREFIX='bradavice_herbology_v4362';
  const plantDefs={
  ...
  const questKey=()=>window.BradaviceAchievements?.scopedKey?.(QUEST_KEY)||`${QUEST_KEY}_${currentStudentId()}`;
  ...
  function currentStudentId(){const s=readJson(STUDENT_KEY,null);return String(s?.supabaseUserId||s?.email||'guest').replace(/[^a-zA-Z0-9@._-]/g,'_')}
  const herbKey=()=>`${HERB_PREFIX}_${currentStudentId()}`;
  function readHerb(){const s=readJson(herbKey(),defaultState())||defaultState();s.found||={};return s}
  function saveHerb(s){s.updatedAt=new Date().toISOString();writeJson(herbKey(),s)}
  function mirrorQuest(id){const q=readQuest();q[id]={done:true,at:new Date().toISOString()};writeJson(questKey(),q);writeJson(QUEST_KEY,q)}
  ...
  async function syncBeforeRender(){
    ...
    try{
      ...
      if(DB?.client&&DB?.hydrateProgress)synced=Boolean(await DB.hydrateProgress());
    }catch(e){console.warn('Prýtová – načtení postupu selhalo:',e)}
  ...
  function initGreenhouse(){
    ...
    function render(){
      ...
      if(herbDone()){A?.award?.('herbarnik',{silent:true});herbDialog.innerHTML='<strong>Herbář je odevzdaný.</strong> Všech pět vzorků má profesorka Prýtová zapsaných.';herbAction.hidden=true}
  ...
  (async()=>{await syncBeforeRender();mountPlants();initGreenhouse();window.addEventListener('bradavice:progress-synced',()=>{reconcileFromQuestRows();refreshMountedPlants();if(page==='skleniky.html')document.getElementById('herbQuestModal')?._herbRender?.()})})();
})();
