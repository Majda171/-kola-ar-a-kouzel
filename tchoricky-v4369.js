(()=>{
 ...
 const W=canvas.width,H=canvas.height,arena={x:W/2,y:H/2,r:286},FRICTION=.985,BOUNCE=.91,STOP=.055,MAX_POWER=21;
 ...
 const boardImg=new Image(),blueSwirl=new Image(),blueStar=new Image(),greenSwirl=new Image(),greenStar=new Image();
 boardImg.src='img/tchoricky-board-v4369.webp';blueSwirl.src='img/tchoricky-blue-swirl-v4369.webp';blueStar.src='img/tchoricky-blue-star-v4369.webp';greenSwirl.src='img/tchoricky-green-swirl-v4369.webp';greenStar.src='img/tchoricky-green-star-v4369.webp';
 ...
 function drawBoard(){
   ...
   const bw=650,bh=642,bx=arena.x-bw/2,by=arena.y-bh/2;
 ...
 function stoneSprite(s){
   ...
   const n=Number(String(s.id||'').split('-').pop())||0;
   if(blue){return (s.type==='heavy'||(s.type==='normal'&&n%2))?blueStar:blueSwirl}
   return (s.type==='heavy'||(s.type==='normal'&&n%2))?greenStar:greenSwirl;
 }
 function drawStone(s){
   const img=stoneSprite(s),size=s.r*(s.type==='heavy'?3.05:2.9);
   ctx.save();ctx.translate(s.x,s.y);ctx.rotate(s.spin*.18);
   ctx.shadowColor='rgba(0,0,0,.82)';ctx.shadowBlur=10;ctx.shadowOffsetY=5;
   if(img.complete&&img.naturalWidth){ctx.drawImage(img,-size/2,-size/2,size,size)}
   else{ctx.beginPath();ctx.arc(0,0,s.r,0,Math.PI*2);ctx.fillStyle=(s.owner==='player'||s.owner==='host')?'#236fbd':'#27884b';ctx.fill()}
   ctx.shadowBlur=0;ctx.shadowOffsetY=0;
   if(s.type==='swift'){ctx.beginPath();ctx.arc(0,0,s.r*1.18,0,Math.PI*2);ctx.strokeStyle='rgba(228,216,157,.62)';ctx.lineWidth=1.5;ctx.setLineDash([3,3]);ctx.stroke();ctx.setLineDash([])}
   if(s.type==='slime'||s.slimed>0){ctx.beginPath();ctx.arc(0,0,s.r*1.2,0,Math.PI*2);ctx.strokeStyle='rgba(139,177,63,.92)';ctx.lineWidth=3;ctx.stroke()}
   ctx.restore();
 }
 function drawAim(){
