(()=>{
  ...
  function state(){const k=findKing(turn),check=k&&attacked(k[0],k[1],enemy(turn)),moves=all(turn);if(!moves.length){ended=true;if(check&&turn==='b'){status.textContent='MAT. Vyhrál/a jsi první zkoušku.';quote.textContent='„Dobře. Figurky tě pustí dál.“';progress.chess=true;save();paintProgress();setTimeout(()=>{showStage('key');buildKeys()},900)}else if(check){status.textContent='Mat. Zkoušku musíš zkusit znovu.';quote.textContent='„Dveře se neotevřou poraženému.“'}else status.textContent='Pat. Zkus novou partii.';return false}status.textContent=turn==='w'?(check?'Jsi v šachu. Najdi obranu.':'Jsi na tahu.'):(check?'Soupeř je v šachu…':'Kamenné figurky přemýšlejí…');return true}
