(()=>{
  ...
  const centers=[{x:31.5,y:31.5},{x:50,y:22.5},{x:68.5,y:33.5}],kp=[{x:31.5,y:48},{x:50,y:39},{x:68.5,y:50}];
  ...
  function finish(){ball.style.display='none';resetKeeper();status.textContent=`Konec: ${saves} z ${TOTAL} střel chycených.`;flash(saves>=16?'VÝBORNĚ':saves>=10?'DOBRÁ PRÁCE':'TRÉNUJ DÁL');start.textContent='Hrát znovu';if(saves>=12)window.BradaviceAchievements?.award?.('famfrpalova-hvezda',{silent:true});window.BradaviceDB?.submitTournamentScore?.('keeper-cup',Math.round(saves/TOTAL*100)).catch?.(()=>{})}
