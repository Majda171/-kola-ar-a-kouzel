(()=>{
  ...
  function finish(){ball.style.display='none';resetKeeper();status.textContent=`Konec: ${saves} z ${TOTAL} střel chycených.`;flash(saves>=8?'VÝBORNĚ':saves>=5?'DOBRÁ PRÁCE':'TRÉNUJ DÁL');start.textContent='Hrát znovu';if(saves>=6)window.BradaviceAchievements?.award?.('famfrpalova-hvezda',{silent:true});window.BradaviceDB?.submitTournamentScore?.('keeper-cup',Math.round(saves/TOTAL*100)).catch?.(()=>{})}
