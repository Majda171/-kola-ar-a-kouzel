(()=>{
  ...
  function buildBottles(){
    ...
    box.querySelectorAll('button').forEach(b=>b.onclick=()=>{const pos=Number(b.dataset.pos);if(pos===5){msg.textContent='Správně. Zelený lektvar je jediná cesta vpřed.';progress.potion=true;save();paintProgress();box.querySelectorAll('button').forEach(x=>x.disabled=true);window.BradaviceAchievements?.award?.('tajemstvi-hradu',{silent:true});setTimeout(()=>showStage('done'),900)}else{msg.textContent='Špatně. Indicie téhle lahvičce neodpovídají.';b.classList.remove('wrong');void b.offsetWidth;b.classList.add('wrong')}})
  }
  resume();
})();
