(()=>{
  ...
  function reconcileFromQuestRows(authoritative=false){
    ...
    const assign=(key,val)=>{if(authoritative){if(Boolean(s[key])!==Boolean(val)){s[key]=Boolean(val);changed=true}}else if(val&&!s[key]){s[key]=true;changed=true}};
    assign('accepted',rowAccepted);assign('done',rowDone);assign('mandAccepted',rowMandAccepted);assign('mandDone',rowMandDone);
    for(const id of ids){if(authoritative){if(Boolean(s.found[id])!==Boolean(rowFound[id])){s.found[id]=Boolean(rowFound[id]);changed=true}}else if(rowFound[id]&&!s.found[id]){s.found[id]=true;changed=true}}
  ...
  async function completeRemote(id){
    ...
    finally{reconcileFromQuestRows(Boolean(DB?.client))}
  }
  ...
  function initGreenhouse(){
    ...
    function render(){
      reconcileFromQuestRows(false);renderProgress();const n=foundCount();
  ...
  (async()=>{await syncBeforeRender();mountPlants();initGreenhouse();window.addEventListener('bradavice:progress-synced',()=>{reconcileFromQuestRows(true);refreshMountedPlants();if(page==='skleniky.html')document.getElementById('herbQuestModal')?._herbRender?.()})})();
})();
