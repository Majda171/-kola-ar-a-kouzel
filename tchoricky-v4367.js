(()=>{
 ...
 const W=canvas.width,H=canvas.height,arena={x:W/2,y:H/2,r:294},FRICTION=.985,BOUNCE=.91,STOP=.055,MAX_POWER=21;
 ...
 function stoneRadius(type){return type==='heavy'?20:type==='swift'?15.5:17}
 ...
 function formation(owner,sign){
   const out=[],xs=[-112,-56,0,56,112],ys=[178,224,270];
 ...
 function setFromState(st){if(!st||!Array.isArray(st.stones))return;stones=st.stones.map(x=>({...x,r:Number(x.r)||stoneRadius(x.type),vx:0,vy:0,alive:x.alive!==false}));turn=st.turn||turn;phase=st.ended?'over':'aim';updateHud();if(st.ended)showResult(st.winner)}
 function serialState(ended=false,winner=null){return{stones:stones.map(({owner,x,y,r,type,alive,id,slimed})=>({owner,x,y,r,type,alive,id,slimed:slimed||0})),turn,ended,winner,move:Date.now()}}
 ...
 function drawBoard(){
   ...
   // Deska je kreslená přímo do canvasu. Žádný obří obrázek ani bílé okraje.
   const bg=ctx.createRadialGradient(arena.x,arena.y,40,arena.x,arena.y,520);bg.addColorStop(0,'rgba(45,49,43,.96)');bg.addColorStop(.65,'rgba(20,24,20,.97)');bg.addColorStop(1,'rgba(5,8,6,.99)');ctx.fillStyle=bg;ctx.fillRect(0,0,W,H);
   ctx.save();ctx.translate(arena.x,arena.y);
   ctx.shadowColor='rgba(0,0,0,.75)';ctx.shadowBlur=28;ctx.beginPath();ctx.arc(0,0,arena.r+18,0,Math.PI*2);ctx.fillStyle='#151813';ctx.fill();ctx.shadowBlur=0;
   const stone=ctx.createRadialGradient(-80,-100,20,0,0,arena.r);stone.addColorStop(0,'#3b3e38');stone.addColorStop(.48,'#292c28');stone.addColorStop(1,'#171a17');ctx.beginPath();ctx.arc(0,0,arena.r,0,Math.PI*2);ctx.fillStyle=stone;ctx.fill();
   ctx.lineWidth=4;ctx.strokeStyle='#9d824c';ctx.stroke();
   ctx.lineWidth=1.6;ctx.strokeStyle='rgba(197,164,91,.78)';for(const rr of [arena.r*.36,arena.r*.67]){ctx.beginPath();ctx.arc(0,0,rr,0,Math.PI*2);ctx.stroke()}
   ctx.beginPath();ctx.moveTo(-arena.r,0);ctx.lineTo(arena.r,0);ctx.moveTo(0,-arena.r);ctx.lineTo(0,arena.r);ctx.stroke();
   // Čtyři malé mosazné kruhy a středová hvězda dávají desce vzhled staré školní hry.
   for(const [x,y] of [[0,-arena.r*.67],[arena.r*.67,0],[0,arena.r*.67],[-arena.r*.67,0]]){ctx.beginPath();ctx.arc(x,y,24,0,Math.PI*2);ctx.fillStyle='#262923';ctx.fill();ctx.lineWidth=3;ctx.strokeStyle='#a48950';ctx.stroke()}
   ctx.save();ctx.rotate(Math.PI/8);ctx.fillStyle='#a98a4e';ctx.beginPath();for(let i=0;i<16;i++){const r=i%2===0?35:11,a=i*Math.PI/8;const x=Math.cos(a)*r,y=Math.sin(a)*r;i?ctx.lineTo(x,y):ctx.moveTo(x,y)}ctx.closePath();ctx.fill();ctx.restore();
   // jemná kresba kamene
   ctx.globalAlpha=.13;ctx.strokeStyle='#d8d1bd';ctx.lineWidth=1;for(let i=0;i<34;i++){const a=rnd(0,Math.PI*2),r1=rnd(55,arena.r-20),len=rnd(12,38);ctx.beginPath();ctx.moveTo(Math.cos(a)*r1,Math.sin(a)*r1);ctx.lineTo(Math.cos(a)*r1+Math.cos(a+1.25)*len,Math.sin(a)*r1+Math.sin(a+1.25)*len);ctx.stroke()}ctx.restore();
 }
 function ownerPalette(owner){
   const mine=owner===myOwner();return mine?['#e4d2a0','#7b5522','#f5e7bd']:['#536b61','#172e26','#93ad9d'];
 }
 function drawStone(s){
   const [a,b,c]=ownerPalette(s.owner),g=ctx.createRadialGradient(s.x-s.r*.35,s.y-s.r*.4,2,s.x,s.y,s.r*1.2);g.addColorStop(0,c);g.addColorStop(.45,a);g.addColorStop(1,b);
   ctx.save();ctx.translate(s.x,s.y);ctx.rotate(s.spin);ctx.beginPath();ctx.arc(0,0,s.r,0,Math.PI*2);ctx.fillStyle=g;ctx.shadowColor='#000';ctx.shadowBlur=9;ctx.shadowOffsetY=5;ctx.fill();ctx.shadowBlur=0;ctx.lineWidth=1.5;ctx.strokeStyle='rgba(238,216,161,.6)';ctx.stroke();
   ctx.fillStyle=s.owner===myOwner()?'#4b351a':'#d4c59d';ctx.font=`700 ${Math.max(10,s.r*.8)}px Georgia`;ctx.textAlign='center';ctx.textBaseline='middle';
   if(s.type==='heavy')ctx.fillText('◆',0,1);else if(s.type==='swift')ctx.fillText('✦',0,1);else if(s.type==='slime')ctx.fillText('●',0,1);
   if(s.slimed>0){ctx.beginPath();ctx.arc(0,0,s.r*.72,0,Math.PI*2);ctx.strokeStyle='rgba(123,155,56,.9)';ctx.lineWidth=3;ctx.stroke()}
   ctx.restore();
 }
 function drawAim(){
 ...
 function resolveCollisions(){
   ...
   for(let i=0;i<alive.length;i++)for(let j=i+1;j<alive.length;j++){
     ...
     if(a.type==='slime'&&a.owner!==b.owner)b.slimed=Math.max(b.slimed||0,2);if(b.type==='slime'&&a.owner!==b.owner)a.slimed=Math.max(a.slimed||0,2);
   }
 }
 function checkOut(){
 ...
 function physics(){
   if(phase==='moving'||phase==='ai-moving'){
     let moving=false;for(const s of stones){if(!s.alive)continue;s.x+=s.vx;s.y+=s.vy;s.spin+=s.rotV;const f=s.slimed>0?.994:FRICTION;s.vx*=f;s.vy*=f;s.rotV*=.99;if(Math.abs(s.vx)<STOP)s.vx=0;if(Math.abs(s.vy)<STOP)s.vy=0;if(Math.hypot(s.vx,s.vy)>0)moving=true}
