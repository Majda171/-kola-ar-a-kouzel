(() => {
  ...
  function summon(){
    ...
    const anim = img.animate([
      ...
      {offset:.14,transform:`translate3d(${startX+(endX-startX)*.14}px,0,0) rotate(${tilt*.5}deg) scale(1)`,opacity:.52},
      {offset:.55,transform:`translate3d(${startX+(endX-startX)*.55}px,${drift}px,0) rotate(${-tilt*.5}deg) scale(1.03)`,opacity:.62},
      {offset:.88,transform:`translate3d(${startX+(endX-startX)*.88}px,${drift*.25}px,0) rotate(${-tilt}deg) scale(1)`,opacity:.48},
